...
import { getdata } from "../redux/slices/thedata";
import { putdata } from "../redux/slices/thedata";
import { changedata } from "../redux/slices/thedata";
export default function UserData() {
    // {data.author.data.name}
    // #####################################################################################
    const data = useContext(User);
    const dispatshRedux = useDispatch()
    // //////////////////////////////////////////////////////////////////////////////////////
    // //////////////////////////////////////////////////////////////////////////////////////
    let initFilter = {
        appointmentDate:"",
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        // del = 0 / add = 1 
        const {dataredux} = useSelector((state)=> state.thedata)
        const len = Math.ceil(dataredux.length / 10)
        ...
        <div className="userdata">
            ...
            <div ref={menu} className="moreinfo"> 
                ...
                <button onClick={()=>{
                    /////////////////// important ////////////////////////////
                            ...
                            if (stateOfModifier === 1) {
                                dispatshRedux(putdata(valueChageReduce))
                            } else if (stateOfModifier === 0) {
                                dispatshRedux(changedata(valueChageReduce))
                            }
                            // console.log(valueChageReduce)
                ...
                <form action="" onSubmit={handlsubmitforms} className="changeAdd">
                    ...
                    <input name="patientId" type="number" value={ stateOfModifier === 0 ?  valueChageReduce?.patientId : stateOfModifier === 1 ? dataredux?.length + 1 : null  } readOnly  />
                ...
                <form action="" onSubmit={handlsubmitforms} className="changeAdd">
                    ...
                    <button onClick={(e)=>{
                        setTypeOfAdding('billing_information')
                    }}>show more</button>
                    <label htmlFor="">consent forms</label>
                    <button onClick={(e)=>{
                        setTypeOfAdding('consent_forms')
                    }}>show more</button>
                </form>
                    ...
                    </tr>
                    {
                        dataredux.map((ele ,i)=> {
                            ...
                            if (!data) {
                                ...
                                // console.log(data)
                                // 1
                                if (filtervaluesreducer.patientId && filtervaluesreducer.patientName && !filtervaluesreducer.appointmentDate) {
                                    if (ele.patientId === parseInt((filtervaluesreducer.patientId)) && (ele.patientName.includes((filtervaluesreducer.patientName)))) {
                                ...
                                // 2
                                if (filtervaluesreducer.patientId && filtervaluesreducer.appointmentDate && !filtervaluesreducer.patientName) {
                                    if (ele.patientId === parseInt((filtervaluesreducer.patientId)) && (ele.appointmentDate.includes((filtervaluesreducer.appointmentDate)))) {
                                        return theresult(ele)
                                    }
                                    else {
                                        return null
                                ...
                                // 3
                                if (filtervaluesreducer.patientName && filtervaluesreducer.appointmentDate && !filtervaluesreducer.patientId ) {
                                    if (ele.patientName.includes((filtervaluesreducer.patientName)) && (ele.appointmentDate.includes((filtervaluesreducer.appointmentDate)))) {
                                        return theresult(ele)
                                    }
                                    else {
                                        return null
                                ...
                                // 4
                                if (filtervaluesreducer.patientId && filtervaluesreducer.patientName && filtervaluesreducer.appointmentDate ) {
                                    if ((ele.patientId === parseInt((filtervaluesreducer.patientId))) &&  ele.patientName.includes((filtervaluesreducer.patientName)) && (ele.appointmentDate.includes((filtervaluesreducer.appointmentDate)))) {
                                        return theresult(ele)
                                    }
                                    else {
                                        return null
                                ...
                                // 5
                                if (filtervaluesreducer.patientId && !filtervaluesreducer.patientName && !filtervaluesreducer.appointmentDate) {
                                    if (ele.patientId === parseInt((filtervaluesreducer.patientId))) {
                                        return theresult(ele)
                                    }else {
                                        return null
                                    }
                                ...
                                // 6
                                if (!filtervaluesreducer.patientId && filtervaluesreducer.patientName && !filtervaluesreducer.appointmentDate ) {
                                    if (ele.patientName.includes((filtervaluesreducer.patientName))) {
                                        return theresult(ele)
                                    }else {
                                        return null
                                    }
                                ...
                                // 7
                                if (!filtervaluesreducer.patientId && !filtervaluesreducer.patientName && filtervaluesreducer.appointmentDate ) {
                                    if (ele.appointmentDate.includes((filtervaluesreducer.appointmentDate))) {
                                        return theresult(ele)
                                    }else {
                                        return null
                                    }
